...
async fn handle_list_version_folder(
    ...
    let stage_entity = match stage_manager.get_entity_by_id(stage_id, &vec![], &vec![]).await {
        Ok(r) => r,
        Err(_e) => {
    ...
    let specs_id = stage_entity
        ...
        .unwrap()
        .to_string();
    let _specs_entity = match specses_manager.get_entity_by_id(&specs_id, &vec![], &vec![]).await {
        Ok(r) => r,
        Err(_e) => {
            return Err(Status::not_found(format!(
                "{}: {}",
