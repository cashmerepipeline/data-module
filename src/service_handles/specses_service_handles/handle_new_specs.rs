...
async fn validate_request_params(
    ...
    match majordomo_arc.get_manager_by_id(*manage_id) {
        Ok(m) => {
            ...
            // 实体不存在
            if !m.entity_exists(&query_doc).await{
                return Err(Status::not_found(format!("{}: {}-{}", t!("实体不存在"), manage_id, entity_id)));
            };
        },
        Err(_) => return Err(Status::not_found(format!("{}: {}", t!("管理不存在"), manage_id))),
    };
...
async fn handle_new_specs(
    ...
    new_entity_doc.insert(SPECSES_TARGETS_FIELD_ID.to_string(), attributes.iter().map(|t| bson::from_slice(t).unwrap()).collect::<Vec<bson::Bson>>());
